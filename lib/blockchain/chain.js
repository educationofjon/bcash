 ...
 * https://github.com/cashnode/nodecash
 */
...
class Chain extends AsyncEmitter {
  ...
  calculateASERT(first, last) {
    ...
    const targetWindow = 600;

    const height = last.height + 1;

    exponent = ((actualTimespan - pow.targetSpacing) * (height * 65536) / tau);

    shifts = exponent >> 16;

    exponent = Math.floor(exponent - (shifts * 65536));

    exp = new BN(exponent);

    factor = new BN(195766423245049).mul(exp).add(new BN(971821376)).muln(5127).addn(2)
    factor.ushrn(48);
    ...
    if (shifts < 0) {
      target = target.ushrn(-shifts);
    } else {
      target = target.ushln(shifts);
    }
